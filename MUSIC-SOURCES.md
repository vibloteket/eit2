...
Reviewed **2026-09-19** for v0.3.7 (music unchanged from v0.3.6; Badinerie audit retained from v0.3.2). This record distinguishes public-domain music,
...
All15 current effects are generated by `scripts/generate-audio/main.go` using
mathematical oscillators, envelopes and deterministic noise, without imported
recordings or MIDI. The original14 effects remain byte-identical. v0.3.7 adds
`count-in.wav`: a55ms wooden tap (1180Hz wood model plus a short noise transient),
played three times by the count-in presentation. Inventory and generator hashes
are in `music/procedural-audio-audit.json`, and tests cover every embedded WAV.
The tap is an effect: Music-off does not silence it; master Mute does.
All five existing music WAVs are unchanged.
