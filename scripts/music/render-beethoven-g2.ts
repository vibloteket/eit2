// Beethoven G2: contrasting, previously unused episodes at 112 BPM.
...
const config = await Bun.file(sourceDir + "/render-config-g2.json").json();
const manifest = await Bun.file(sourceDir + "/sample-sources-g2.json").json();
...
const score = await Bun.file(sourceDir + "/score-g2.json").json();
...
async function section(
  ...
  const [from, to] = score.sections[kind];
...
for (let pass = 0; pass < 2; pass++) {
  ...
  for (let i = 0; i < score.form.length; i++) {
    const before = at,
      kind = score.form[i];
    at = await section({ L, R }, kind, at, false);
    if (pass === 0)
      form.push({
        section: kind,
        repeat: false,
...
// Level the new material near G1 without changing G1; keep peak headroom.
const gain = Math.min(0.82 / peak, config.targetRms / rawRms);
const calibrating = Bun.argv.includes("--calibrate");
if (peak * gain >= 0.9)
  throw Error("Insufficient headroom with unchanged master gain");
const loop = wav(loopL, loopR, gain);
if (!calibrating && digest(loop) !== config.expectedLoopSHA256)
  throw Error("Gameplay loop hash differs; game asset not replaced");
await Bun.write(output + "/beethoven-gameplay-G2-112-loop.wav", loop);
// Listening export covers the complete G2 sequence once, with an ending tail.
const previewFrames = frameAt(202) + sr,
  p = {
...
if (at !== 202) throw Error("Preview length");
ambience(p.L, p.R, false);
const preview = wav(p.L, p.R, gain);
if (!calibrating && digest(preview) !== config.expectedPreviewSHA256)
...
const report = {
  ...
  expectedHashesVerified: !calibrating,
  requestedAdditionalTrack: true,
  assistantAuditioned: false,
};
await Bun.write(
  output + "/beethoven-g2-render-report.json",
