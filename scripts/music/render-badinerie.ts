// Selected after user feedback 2026-09-15: low-flute onset correction, same musical arrangement.
import { mkdir } from "node:fs/promises";
import { createHash } from "node:crypto";
const root = import.meta.dir + "/../..";
const sourceDir = root + "/music/badinerie";
const output = root + "/dist/audio";
const cacheRoot = root + "/.tmp/music-sources";
const config = await Bun.file(sourceDir + "/render-config.json").json();
const manifest = await Bun.file(sourceDir + "/sample-sources.json").json();
const hashes = new Map(manifest.files.map((f) => [f.path, f]));
const digest = (b: Uint8Array) => createHash("sha256").update(b).digest("hex");
await mkdir(output, { recursive: true });
// Full AABB arrangement retaining B2's sample palette, tempo, articulation and ambience.
// Score.json carries the expanded melody, own sparse harmony and source-audit caveat.
const referenceRms = config.referenceRms;
const fluteTrims = new Map<number, number>();
const trimReport: any[] = [];
await mkdir(cacheRoot, { recursive: true });
async function get(path: string) {
  const entry = hashes.get(path) as { url: string; sha256: string } | undefined;
  if (!entry) throw Error("Undeclared sample source: " + path);
  let file = Bun.file(cacheRoot + "/" + path);
  if (!(await file.exists())) {
    const response = await fetch(entry.url);
    if (!response.ok) throw Error(path + ": HTTP " + response.status);
    const data = new Uint8Array(await response.arrayBuffer());
    if (digest(data) !== entry.sha256)
      throw Error("Downloaded source hash mismatch: " + path);
    await Bun.write(cacheRoot + "/" + path, data);
    file = Bun.file(cacheRoot + "/" + path);
  }
  if (digest(new Uint8Array(await file.arrayBuffer())) !== entry.sha256)
    throw Error("Cached source hash mismatch: " + path);
  return file;
}
await get("LICENSE");
type Region = {
  sample: string;
  center: number;
  lo: number;
  hi: number;
  velLo: number;
  velHi: number;
};
async function instrument(name: string, velocity: number) {
  const text = await (await get(name + ".sfz")).text();
  const path = text
    .match(/default_path=(.*)/)![1]
    .trim()
    .replaceAll("\\", "/");
  const out: Region[] = [];
  for (const block of text.split("<region>").slice(1)) {
    const val = (k: string, d = 0) =>
      Number(block.match(new RegExp("\\b" + k + "=([^\\s]+)"))?.[1] ?? d);
    const sample = block.match(/sample=([^\r\n]+)/)?.[1].trim();
    if (!sample) continue;
    const r = {
      sample: path + sample.replaceAll("\\", "/"),
      center: val("pitch_keycenter"),
      lo: val("lokey"),
      hi: val("hikey"),
      velLo: val("lovel"),
      velHi: val("hivel", 127),
    };
    if (
      velocity >= r.velLo &&
      velocity <= r.velHi &&
      !out.some((x) => x.center === r.center)
    )
      out.push(r);
  }
  if (!out.length) throw new Error("no regions");
  return out;
}
function wavRead(buffer: ArrayBuffer) {
  const b = Buffer.from(buffer);
  if (b.toString("ascii", 0, 4) !== "RIFF") throw new Error("not RIFF");
  let fmt: any, data: Buffer | undefined;
  for (let p = 12; p + 8 <= b.length; ) {
    const id = b.toString("ascii", p, p + 4),
      n = b.readUInt32LE(p + 4);
    if (id === "fmt ")
      fmt = {
        kind: b.readUInt16LE(p + 8),
        channels: b.readUInt16LE(p + 10),
        rate: b.readUInt32LE(p + 12),
        bits: b.readUInt16LE(p + 22),
      };
    if (id === "data") data = b.subarray(p + 8, p + 8 + n);
    p += 8 + n + (n % 2);
  }
  if (!fmt || !data) throw new Error("bad WAV");
  const { kind, channels, rate, bits } = fmt;
  const bytes = bits / 8,
    n = data.length / bytes / channels;
  const samples = new Float32Array(n);
  for (let i = 0; i < n; i++) {
    let v = 0;
    for (let c = 0; c < channels; c++) {
      const o = (i * channels + c) * bytes;
      v +=
        kind === 3
          ? data.readFloatLE(o)
          : bits === 16
            ? data.readInt16LE(o) / 32768
            : bits === 24
              ? data.readIntLE(o, 3) / 8388608
              : bits === 32
                ? data.readInt32LE(o) / 2147483648
                : (data[o] - 128) / 128;
    }
    samples[i] = v / channels;
  }
  return { samples, rate };
}
const cache = new Map<string, ReturnType<typeof wavRead>>();
const flute = await instrument("FluteStac", 80),
  cello = await instrument("CelloEnsPizz", 50);
const score = await Bun.file(sourceDir + "/score.json").json();
const baseChanges = config.baseCalibration;
const notes = [...score.sections.A, ...score.sections.B].map((m) => m.notes);
const sr = 44100,
  bpm = 108,
  beat = 60 / bpm,
  phraseBeats = 160;
const cycleFrames = phraseBeats * beat * sr;
if (!Number.isInteger(cycleFrames)) throw Error("Nonintegral loop frames");
const seconds = 2 * phraseBeats * beat + 0.5;
// Correct only a quarter of the sample-to-sample level variation, capped at +/-0.8dB.
const keyLevels: any[] = [];
for (const key of [...new Set(notes.flat().map((n) => n.key))]) {
  const r = flute.find((r) => key >= r.lo && key <= r.hi);
  if (!r) throw new Error("Missing flute key");
  let wav = cache.get(r.sample);
  if (!wav) {
    wav = wavRead(await (await get(r.sample)).arrayBuffer());
    cache.set(r.sample, wav);
  }
  let peak = 0;
  for (const x of wav.samples) peak = Math.max(peak, Math.abs(x));
  const ratio = (2 ** ((key - r.center) / 12) * wav.rate) / sr;
  let energy = 0;
  const count = Math.min(
    Math.floor(wav.samples.length / ratio) - 1,
    Math.floor(0.16 * sr),
  );
  for (let i = 0; i < count; i++) {
    const pos = i * ratio,
      k = Math.floor(pos),
      f = pos - k;
    const x =
      (wav.samples[k] * (1 - f) + wav.samples[k + 1] * f) /
      Math.max(peak, 0.01);
    energy += x * x;
  }
  keyLevels.push({
    key,
    normalizedRms: Math.sqrt(energy / count),
    sample: r.sample,
  });
}
const sorted = baseChanges.fluteTrims
    .map((k) => k.normalizedRms)
    .sort((a, b) => a - b),
  median = sorted[Math.floor(sorted.length / 2)];
for (const item of keyLevels) {
  const trimDB =
    baseChanges.fluteTrims.find((k) => k.key === item.key)?.trimDB ??
    Math.max(
      -0.8,
      Math.min(0.8, 20 * Math.log10(median / item.normalizedRms) * 0.25),
    );
  fluteTrims.set(item.key, 10 ** (trimDB / 20));
  trimReport.push({ ...item, trimDB });
}
await Bun.write(
  output + "/changes.json",
  JSON.stringify(
    {
      bpm,
      phraseBeats,
      referenceRms,
      fluteTrims: trimReport,
      changes: {
        fluteReleaseMs: 60,
        fluteAttackMs: 5,
        weakNoteGain: 0.935,
        bassGain: 0.092,
        secondaryBassGain: 0.048,
        pluckGain: 0.0245,
      },
      sourceStatus:
        "Audited public-domain melody and CC0 sample sources; see MUSIC-SOURCES.md.",
    },
    null,
    2,
  ),
);
async function sampled(
  L: Float32Array,
  R: Float32Array,
  regions: Region[],
  key: number,
  start: number,
  dur: number,
  gain: number,
  pan: number,
) {
  let r =
    regions.find((r) => key >= r.lo && key <= r.hi) ??
    regions.reduce((a, b) =>
      Math.abs(a.center - key) < Math.abs(b.center - key) ? a : b,
    );
  let wav = cache.get(r.sample);
  if (!wav) {
    wav = wavRead(await (await get(r.sample)).arrayBuffer());
    cache.set(r.sample, wav);
  }
  let peak = 0;
  for (const x of wav.samples) peak = Math.max(peak, Math.abs(x));
  const ratio = (2 ** ((key - r.center) / 12) * wav.rate) / sr;
  const isFlute = regions === flute,
    releaseTime = isFlute ? 0.06 : 0.055;
  if (isFlute) gain *= fluteTrims.get(key) ?? 1;
  // Remove 50ms of low-flute lead-in, as in the preferred comparison. Same pitches, envelopes and gain.
  const sampleOffset =
    isFlute && r.sample.endsWith("LDFlute_stac_A3_v2_rr1.wav")
      ? Math.round(0.05 * wav.rate)
      : 0;
  let begin = Math.round(start * sr),
    count = Math.min(
      Math.floor((wav.samples.length - sampleOffset) / ratio) - 1,
      Math.floor((dur + releaseTime) * sr),
    );
  const lg = Math.sqrt((1 - pan) / 2),
    rg = Math.sqrt((1 + pan) / 2);
  for (let i = 0; i < count && begin + i < L.length; i++) {
    let pos = sampleOffset + i * ratio,
      k = Math.floor(pos),
      f = pos - k,
      t = i / sr;
    const phase = Math.min(1, Math.max(0, (t - dur) / releaseTime));
    const release = isFlute ? 0.5 + 0.5 * Math.cos(Math.PI * phase) : 1 - phase;
    const attack = isFlute
      ? 0.5 - 0.5 * Math.cos(Math.PI * Math.min(1, t / 0.005))
      : Math.min(1, t / 0.004);
    const v =
      (((wav.samples[k] * (1 - f) + wav.samples[k + 1] * f) * gain) /
        Math.max(peak, 0.01)) *
      release *
      attack;
    L[begin + i] += v * lg;
    R[begin + i] += v * rg;
  }
}
function pluck(
  L: Float32Array,
  R: Float32Array,
  key: number,
  start: number,
  gain: number,
) {
  const freq = 440 * 2 ** ((key - 69) / 12),
    begin = Math.round(start * sr);
  for (let i = 0; i < sr * 0.35 && begin + i < L.length; i++) {
    const t = i / sr;
    const v =
      (Math.sin(2 * Math.PI * freq * t) +
        0.24 * Math.sin(4 * Math.PI * freq * t) +
        0.1 * Math.sin(6 * Math.PI * freq * t)) *
      Math.exp(-t * 15) *
      Math.min(1, t / 0.003) *
      gain;
    L[begin + i] += v * 0.58;
    R[begin + i] += v * 0.82;
  }
}

// Same B2 palette, with very small omissions on written repeats, not added voices.
const framePerBeat = (sr * 60) / bpm;
function atSeconds(b: number) {
  return Math.round(b * framePerBeat) / sr;
}
const totalFrames = Math.ceil(seconds * sr),
  L = new Float32Array(totalFrames),
  R = new Float32Array(totalFrames);
const formReport: any[] = [],
  eventCounts = { flute: 0, bass: 0, pluck: 0 };
let maxDryEnd = 0;
for (let pass = 0; pass < 2; pass++) {
  let position = pass * phraseBeats;
  for (let formIndex = 0; formIndex < score.form.length; formIndex++) {
    const section = score.form[formIndex],
      bars = score.sections[section],
      repeat = formIndex % 2 === 1;
    const beginning = position;
    for (let b = 0; b < bars.length; b++) {
      const bar = bars[b],
        barStart = position;
      let local = 0,
        index = 0;
      for (const n of bar.notes) {
        if (n.key !== null) {
          const start = atSeconds(barStart + local),
            dur = n.beats * beat * (n.beats < 0.5 ? 0.88 : 0.76);
          await sampled(
            L,
            R,
            flute,
            n.key,
            start,
            dur,
            0.3 * (index++ % 4 === 0 ? 1 : 0.935),
            -0.08,
          );
          eventCounts.flute++;
          maxDryEnd = Math.max(maxDryEnd, start + dur + 0.06);
        }
        local += n.beats;
      }
      if (Math.abs(local - bar.beats) > 1e-9)
        throw Error("Bar duration mismatch");
      // As in B2, leave the section's upbeat unaccompanied.
      if (b > 0) {
        const c0 = bar.chords[0];
        await sampled(
          L,
          R,
          cello,
          c0[0],
          atSeconds(barStart),
          0.24,
          0.092,
          0.15,
        );
        eventCounts.bass++;
        if (bar.beats >= 2 && !(repeat && b % 4 === 3)) {
          const bass = section === "A" ? c0[0] + 7 : (bar.chords[1] ?? c0)[0];
          await sampled(
            L,
            R,
            cello,
            bass,
            atSeconds(barStart + 1),
            0.2,
            0.048,
            0.15,
          );
          eventCounts.bass++;
        }
        for (let q = 0; q < bar.beats; q += 0.5) {
          if (repeat && b % 4 === 3 && q === bar.beats - 0.5) continue;
          const chord =
            bar.chords[Math.min(Math.floor(q), bar.chords.length - 1)];
          pluck(
            L,
            R,
            chord[1 + (Math.round(q * 2) % 2)] + 12,
            atSeconds(barStart + q),
            0.0245 * (repeat ? 0.98 : 1),
          );
          eventCounts.pluck++;
        }
      }
      position += bar.beats;
    }
    if (pass === 0)
      formReport.push({
        section,
        repeat,
        startBeat: beginning,
        endBeat: position,
        seconds: (position - beginning) * beat,
        measures: bars.map((b) => b.measure),
      });
  }
  if (position !== (pass + 1) * phraseBeats)
    throw Error("Full form duration mismatch");
}
if (maxDryEnd > 2 * phraseBeats * beat)
  throw Error("Unexpected dry voice overhang; handle before looping");
const coldL = L.slice(0, cycleFrames),
  coldR = R.slice(0, cycleFrames);
const endTailL = L.slice(2 * cycleFrames),
  endTailR = R.slice(2 * cycleFrames);
const dryFirstL = coldL.slice(),
  dryFirstR = coldR.slice();
// Final pluck releases cross the boundary; fold their exact tail, not a crossfade.
for (let i = 0; i < endTailL.length; i++) {
  dryFirstL[i] += endTailL[i];
  dryFirstR[i] += endTailR[i];
}
let dryCycleError = 0;
for (let i = 0; i < cycleFrames; i++)
  dryCycleError = Math.max(
    dryCycleError,
    Math.abs(dryFirstL[i] - L[i + cycleFrames]),
    Math.abs(dryFirstR[i] - R[i + cycleFrames]),
  );
if (dryCycleError > 2e-7)
  throw Error(
    "Dry-cycle error exceeds float accumulation tolerance: " + dryCycleError,
  );
function ambience(l: Float32Array, r: Float32Array, circular: boolean) {
  for (const [a, b] of [
    [l, r],
    [r, l],
  ]) {
    const dry = a.slice();
    for (const d of [0.037, 0.061, 0.089]) {
      const offset = Math.floor(d * sr);
      for (let i = circular ? 0 : offset; i < a.length; i++)
        b[i] += dry[(i - offset + a.length) % a.length] * 0.035;
    }
  }
}
ambience(L, R, false);
const loopL = L.slice(cycleFrames, 2 * cycleFrames),
  loopR = R.slice(cycleFrames, 2 * cycleFrames);
const circularL = dryFirstL.slice(),
  circularR = dryFirstR.slice();
ambience(circularL, circularR, true);
let circularError = 0;
for (let i = 0; i < cycleFrames; i++)
  circularError = Math.max(
    circularError,
    Math.abs(loopL[i] - circularL[i]),
    Math.abs(loopR[i] - circularR[i]),
  );
if (circularError > 2e-7)
  throw Error(
    "Cyclic-tail validation exceeds float accumulation tolerance: " +
      circularError,
  );
let peak = 0,
  energy = 0;
for (let i = 0; i < cycleFrames; i++) {
  peak = Math.max(peak, Math.abs(loopL[i]), Math.abs(loopR[i]));
  energy += loopL[i] * loopL[i] + loopR[i] * loopR[i];
}
const referenceGain = config.masterGain;
const gain = Math.min(referenceGain, 0.85 / peak);
function wav(l: Float32Array, r: Float32Array, master: number) {
  const data = Buffer.alloc(44 + l.length * 4);
  data.write("RIFF", 0);
  data.writeUInt32LE(data.length - 8, 4);
  data.write("WAVEfmt ", 8);
  data.writeUInt32LE(16, 16);
  data.writeUInt16LE(1, 20);
  data.writeUInt16LE(2, 22);
  data.writeUInt32LE(sr, 24);
  data.writeUInt32LE(sr * 4, 28);
  data.writeUInt16LE(4, 32);
  data.writeUInt16LE(16, 34);
  data.write("data", 36);
  data.writeUInt32LE(l.length * 4, 40);
  for (let i = 0; i < l.length; i++)
    for (const [c, a] of [l, r].entries()) {
      const v = a[i] * master;
      if (!Number.isFinite(v) || Math.abs(v) >= 1)
        throw Error("Clipping/nonfinite sample");
      data.writeInt16LE(Math.round(v * 32767), 44 + i * 4 + c * 2);
    }
  return data;
}
const loop = wav(loopL, loopR, gain);
if (digest(loop) !== config.expectedLoopSHA256)
  throw Error(
    "Rendered loop differs from approved hash; existing game asset left untouched",
  );
await Bun.write(output + "/badinerie-B2-full-lobby-loop.wav", loop);
// External listening preview: a cold start, complete AABB and actual decay, no next-cycle notes.
const previewFrames = cycleFrames + Math.round(1.8 * sr),
  previewL = new Float32Array(previewFrames),
  previewR = new Float32Array(previewFrames);
previewL.set(coldL);
previewR.set(coldR);
previewL.set(endTailL, cycleFrames);
previewR.set(endTailR, cycleFrames);
ambience(previewL, previewR, false);
const listening = wav(previewL, previewR, gain);
if (digest(listening) !== config.expectedListeningSHA256)
  throw Error("Rendered listening version differs from approved hash");
await Bun.write(output + "/badinerie-B2-full-listening.wav", listening);
await Bun.write(root + "/internal/sound/audio/lobby-badinerie.wav", loop);
const report = {
  bpm,
  beats: phraseBeats,
  seconds: cycleFrames / sr,
  frames: cycleFrames,
  sampleRate: sr,
  form: formReport,
  eventCounts,
  uniqueSamples: cache.size,
  masterGain: gain,
  referenceGain,
  masterAdjustmentDB: 20 * Math.log10(gain / referenceGain),
  peakDBFS: 20 * Math.log10(peak * gain),
  rmsDBFS: 20 * Math.log10(Math.sqrt(energy / (cycleFrames * 2)) * gain),
  circularError,
  dryCycleError,
  cycleTolerance: 2e-7,
  expectedHashesVerified: true,
  noNewInstruments: true,
  sourceStatus:
    "Audited public-domain melody and CC0 sample sources; see MUSIC-SOURCES.md.",
};
await Bun.write(
  output + "/render-report.json",
  JSON.stringify(report, null, 2),
);
console.log(JSON.stringify(report, null, 2));
